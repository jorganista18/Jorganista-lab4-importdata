...
```{r}
#| label: load-data
buoy2024 <- read_tsv("data-raw/buoy_2024.txt")
```
...
buoy2024_row1 <- read_tsv("data-raw/buoy_2024.txt",
                          comment = "#")
```
...
```{r}
buoy2024_clean<- janitor::clean_names(buoy2024_row1)
```
...
ggplot(buoy_final, aes(x = ___, y = ____, color = factor(___)) +
