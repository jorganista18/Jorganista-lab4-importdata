...
```{r}
#| label: load-data
buoy2024 <- read_table("data-raw/buoy_2024.txt")
...
buoy2024_row1 <- read_table("data-raw/buoy_2024.txt",
                          comment = "#")
print(buoy2024_row1)
```
...
```{r}
buoy2024_clean<- read_table("data-raw/buoy_2024.txt",
                          comment = "#") %>%
  clean_names()

print(buoy2024_clean)
```
...
buoy_update.csv <- write_csv(buoy2024_clean, file = "data-raw/buoy_update.csv")
```
...
buoy_new <- read_csv("data-raw/buoy_update.csv",
...
```{r}
#| eval: false
buoy_final <- write_csv(buoy_final, "data-clean/buoy_final")
```
...
ggplot(buoy_final, aes(x = mm, y = atmp, color = factor(mm))) +
...
This data set is a collection of many natural disasters in various countries of the world. There are many variable that can be useful to what I think I can research, clean, and code so this data will be interesting to learn from.

https://www.kaggle.com/datasets/brsdincer/all-natural-disasters-19002021-eosdis

1. Does the country of origin affect the amount of fatalities/ damage costs?

2. How does the type of disaster differ in the result of the area that it affected?
